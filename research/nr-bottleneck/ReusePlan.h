...
namespace Bottleneck
{
    ...
    // One plan belongs to exactly one feature generation. The oracle serializes
    // recording/submission/completion; this is NOT a production frame scheduler.
    // A mismatch throws before submission. Never submit a partially filtered list.
    class ReusePlan
    {
    public:
        void Begin(bool reset, bool enabled)
        {
            Need(!recording_ && !pending_, "Previous evaluation not retired");
            current_.clear();
        ...
        void Retired()
        {
            Need(pending_ && !recording_, "Evaluation not ready for retirement");
            pending_ = false;
