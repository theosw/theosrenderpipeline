#include "FrameGen/NeuralRenderingBottleneckPlan.h"
namespace Bottleneck = TheosRenderPipeline::NeuralRendering::Bottleneck;
#include <iostream>
...
int main() try
{
    const auto records = Fixture();
    {
        ...
        NR::Reconstruction original, toggled;
        Check(!original.bottleneckReuse);
        toggled.bottleneckReuse = true;
        Check(!NR::SameReconstructionResources(original, toggled));
        Check(NR::SanitizeReconstruction(toggled).bottleneckReuse);
    }
    ...
    { // Recording does not make a result reusable before successful same-queue submission.
        ReusePlan plan; Evaluate(plan, records, false, true, false);
        Reject([&] { plan.Begin(false, true); });
        plan.Submitted(); Check(Evaluate(plan, records, false, true) == 42);
    ...
    for (unsigned mutation = 0; mutation != 8; ++mutation) {
        ...
        // In the renderer this failed recording is discarded, never submitted.
    ...
    std::cout << "PASS: alternation, reset, toggles, submission, recreation, malformed order, bindings and alias guards\n";
    return 0;
